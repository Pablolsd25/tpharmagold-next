    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-zinc-500 text-sm mt-1">Panel de control Empire Nutrition</p>
        </div>
        <Link
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          <div className="flex items-start justify-between mb-4">
            ...
            <svg width="14" height="14" className="text-zinc-700 group-hover:text-zinc-500 transition-colors" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <polyline points="9 18 15 12 9 6" />
            </svg>
          </div>
          <p className="text-accent font-display font-bold text-3xl leading-none">{totalProducts ?? 0}</p>
        ...
        >
          ...
          <p className="text-blue-400 font-display font-bold text-3xl leading-none">{totalOrders ?? 0}</p>
          <p className="text-white font-medium text-sm mt-1.5">Órdenes</p>
          <p className="text-zinc-600 text-xs mt-0.5">historial completo</p>
        </Link>

        {/* Ingresos */}
        <Link href="/admin/ordenes?status=paid"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-800">
            <h2 className="text-white font-display font-semibold uppercase tracking-wide text-sm">Órdenes recientes</h2>
            <Link href="/admin/ordenes" className="text-accent text-xs hover:underline flex items-center gap-1">
              Ver todas
          ...
          <div className="md:hidden divide-y divide-zinc-800">
            {(recentOrders ?? []).map((order) => (
              ...
              >
                ...
                <div className="ml-3 flex flex-col items-end gap-1.5">
                  <span className="text-white font-semibold text-sm">${Number(order.total).toLocaleString('es-MX')}</span>
                  <span className={`px-2 py-0.5 rounded-full text-[11px] font-medium border ${STATUS_COLORS[order.status] ?? 'bg-zinc-700 text-zinc-300 border-zinc-700'}`}>
            ...
            {(recentOrders ?? []).length === 0 && (
              <p className="px-5 py-10 text-center text-zinc-600 text-sm">No hay órdenes aún</p>
            )}
          </div>
          ...
          <div className="hidden md:block overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-zinc-800">
                {(recentOrders ?? []).map((order) => (
                  <tr key={order.id} className="hover:bg-zinc-800/30 transition-colors">
                    ...
                    <td className="px-5 py-3 text-center">
                      <span className={`px-2.5 py-0.5 rounded-full text-[11px] font-medium border ${STATUS_COLORS[order.status] ?? 'bg-zinc-700 text-zinc-300 border-zinc-700'}`}>
                        {STATUS_LABELS[order.status] ?? order.status}
                      </span>
                    </td>
                ...
                {(recentOrders ?? []).length === 0 && (
                  <tr><td colSpan={5} className="px-5 py-10 text-center text-zinc-600">No hay órdenes aún</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-800">
            <h2 className="text-white font-display font-semibold uppercase tracking-wide text-sm">Stock bajo</h2>
          ...
          <div className="divide-y divide-zinc-800">
            {(lowStock ?? []).map((p) => (
              ...
              >
                <span className="text-zinc-300 text-sm truncate flex-1 mr-3">{p.name}</span>
                <span className={`text-sm font-bold flex-shrink-0 ${p.stock === 0 ? 'text-red-400' : 'text-yellow-400'}`}>
                  {p.stock === 0 ? 'AGOTADO' : `${p.stock} uds`}
                </span>
              </Link>
            ))}
            {(lowStock ?? []).length === 0 && (
              <p className="px-5 py-8 text-center text-zinc-600 text-sm">✓ Todo en orden</p>
            )}
          </div>
        </div>
      </div>
      ...
      {/* ── Quick actions ──────────────────────────────────── */}
      <div>
        <h2 className="text-zinc-500 text-xs uppercase tracking-widest font-medium mb-3">Accesos rápidos</h2>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { href: '/admin/productos/nuevo', label: 'Nuevo producto',   icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="21 8 21 21 3 21 3 8" /><rect x="1" y="3" width="22" height="5" rx="1" /><line x1="10" y1="12" x2="14" y2="12" /></svg> },
            { href: '/admin/categorias',       label: 'Categorías',       icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20.59 13.41l-7.17 7.17a2 2 0 01-2.83 0L2 12V2h10l8.59 8.59a2 2 0 010 2.82z" /><line x1="7" y1="7" x2="7.01" y2="7" /></svg> },
            { href: '/admin/blog/nuevo',       label: 'Nuevo artículo',   icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 20h9" /><path d="M16.5 3.5a2.121 2.121 0 013 3L7 19l-4 1 1-4L16.5 3.5z" /></svg> },
